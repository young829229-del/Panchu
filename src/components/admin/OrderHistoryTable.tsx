...
export const OrderHistoryTable: React.FC<OrderHistoryTableProps> = ({
  ...
  // Helper to compute display time remaining or elapsed
  const formatTimeDisplay = (order: Order): string => {
  ...
  const renderStatusBadge = (status: OrderStatus) => {
    switch (status) {
      case 'Delivered':
        return (
          <div className="flex items-center gap-1.5 text-xs text-[#faad14] font-medium font-sans">
  ...
  const handleMarkCollected = async (order: Order) => {
    await onUpdateStatus(order.id || order.orderId, 'Collected');
    setActiveMenuOrderId(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Header Section with Heading, Tabs, and Date Pickers matching reference */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-2">
        ...
        {/* Date Range Pickers matching reference */}
      ...
      <div className="overflow-x-auto -mx-2 sm:mx-0">
        <div className="min-w-[760px] inline-block w-full align-middle">
          ...
          {/* Table Header Row */}
          <div className="grid grid-cols-[80px_1.5fr_1fr_1.1fr_1fr_1.1fr_1fr_60px] gap-3 px-4 py-3 text-[11px] font-sans font-medium text-stone-400 border-b border-stone-100">
            <button
          ...
          {sortedOrders.length === 0 ? (
            ...
            <div className="divide-y divide-stone-50/80 my-1">
              {sortedOrders.map((order, index) => {
                ...
                const isDelivery = (order.deliveryOption || '').toLowerCase().includes('door') || 
                                   (order.deliveryOption || '').toLowerCase().includes('road') || 
                                   (order.deliveryOption || '').toLowerCase().includes('delivery') ||
                                   !order.deliveryOption;

                const paymentMethod = order.deliveryOption?.includes('pickup') ? 'Paid' : 'Cash';

                return (
                  <div
                    key={orderKey}
                    className={`grid grid-cols-[80px_1.5fr_1fr_1.1fr_1fr_1.1fr_1fr_60px] gap-3 items-center px-4 py-3.5 rounded-2xl hover:bg-[#faf9f8] transition-colors relative ${
                  ...
                  >
                    ...
                    {/* Customer Name + Avatar matching reference */}
                    <div
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-stone-800 font-sans truncate group-hover:text-[#ff4d4f] transition-colors">
                        {order.customerName}
                      </span>
                    </div>

                    {/* Payment Status */}
                    <div className="text-xs text-stone-600 font-sans font-normal">
                      {paymentMethod}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-stone-500 font-sans">
                      ...
                      <span>{formatTimeDisplay(order)}</span>
                    </div>

                    {/* Type matching reference: coral Delivery or dark Collection */}
                    <div className="text-xs font-medium font-sans">
                      {isDelivery ? (
                        <span className="text-[#ff4d4f]">Delivery</span>
                      ) : (
                    ...
                    <div className="text-right relative">
                      ...
                      {/* Floating Action Menu popup matching reference image exactly */}
                      {isMenuOpen && (
                        <div
                          ref={menuRef}
                          className="absolute right-0 top-8 w-36 bg-white rounded-2xl shadow-[0_12px_32px_rgba(0,0,0,0.12)] border border-stone-100 py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150 text-left"
                        >
                          {/* Refund / Cancel Option */}
                          <button
                            type="button"
                            onClick={() => handleRefundOrCancel(order)}
                            className="w-full px-3.5 py-2 text-xs font-sans text-stone-700 hover:text-red-600 hover:bg-red-50/50 flex items-center gap-2 transition-colors cursor-pointer"
                          >
                            <RotateCcw className="w-3.5 h-3.5 text-stone-400" />
                            <span>Refund</span>
                          </button>

                          {/* Message via WhatsApp / Chat Option */}
                          <button
                            type="button"
                          ...
                          <button
                            ...
                            className="w-full px-3.5 py-2 text-[11px] font-sans text-stone-600 hover:bg-stone-50 flex items-center gap-2 transition-colors cursor-pointer"
                          >
                            <PackageCheck className="w-3 h-3 text-stone-700" />
                            <span>Collected</span>
                          </button>
